...
public class ProcesFragment extends Fragment implements ListTransactionAdapter.onTransactionClick {
    ...
    private void get(){
        apiTransaction.getTransaction( (new Sesi(getActivity()).valid() ? new Sesi(getActivity()).get().getId() : "0"), "0").enqueue(new Callback<Transaction>() {
            @Override
